defmodule RumblAccountsTest do
  ...
  describe "register_user/1" do
    ...
    test "requires password to be at least 6 chars long" do
      attrs = Map.put(@valid_attrs, :password, "12")
      {:error, changeset} = Accounts.register_user(attrs)
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns unauthorized error with invalid password", %{user: user} do
      assert {:error, :unauthorized} =
               Accounts.authenticate_by_username_and_pass(user.username, "badpassword")
    end

    test "returns not found error with no matching user for username" do
      assert {:error, :not_found} =
               Accounts.authenticate_by_username_and_pass("invalid_username", @pass)
    end
  end
end
